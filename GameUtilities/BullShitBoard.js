...
class BullShitBoard {
  ...
  newGame(players) {
    ...
    for (let i = 0; i < numPlayers; i++) {
      this.players[i].playerCards = cardDeck.cards.slice(
        i * divisibleCards,
        (i + 1) * divisibleCards
      );
    }
  ...
  playMove(player, cardIndices) {
    ...
    player.playerCards = player.playerCards.filter(
      (value, index) => !cardIndices.includes(index)
    );
    //set current player
    ...
    if (!this.turnedEnded) {
      ...
      do {
        this.currentPlayerPos =
          (this.currentPlayerPos + 1) % this.players.length;
  ...
  checkBluff(player) {
    ...
    if (
      this.lastPlayedHand.every(
        (card) => card.rank === this.ranks[this.currentCall]
      )
    ) {
      ...
      this.discardPile.forEach((card) =>
        this.currentPlayer.playerCards.push(card)
      );
